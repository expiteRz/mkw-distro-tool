...
impl MainView for CheatCodeApp {
    fn name(&self) -> &'static str {
        "Cheat Code Manager"
    }

    fn ui(&mut self, ctx: &Context) {
        egui::SidePanel::right("cheat_code_manager")
            .resizable(false)
            .show(ctx, |ui| {
                ...
                ui.add_enabled_ui(self.enabled, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        if ui
                            .button("New")
                            .on_hover_text("Add new entry in code list")
                            .clicked()
                        {
                            println!("Code:New");
                        ...
                        ui.with_layout(Layout::right_to_left(Align::Min), |ui| {
                            if ui
                                .button("Delete")
                                .on_hover_text("Remove the selected entry from list")
                                .clicked()
                            {
                                println!("Code:Remove");
                                if self.codes.len() <= 1 {
                                ...
                                self.selected_code = if selected < 1 {
                                    self.selected_code
                                } else {
